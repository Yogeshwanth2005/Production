...
import { Search, Edit3, ArrowLeft, ClipboardCheck, CheckCircle2, ShieldAlert, Link2, LayoutGrid } from 'lucide-react';
...
export default function SealingEntry() {
  ...
  if (view === 'form' && activeBatch) {
    const itemsToSeal = activeBatch.items.filter(i => 
      i.qcStatus === 'QC Passed' || i.itemStatus === 'Sealed' || i.itemStatus === 'Tagged' || i.itemStatus === 'In Inventory'
    );
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 w-full animate-in fade-in duration-300 font-sans">
        ...
        <div className="flex-1 p-8 pt-4 space-y-6 overflow-y-auto">
           <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm flex justify-between items-center max-w-6xl mx-auto">
              <div>
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Seal Progress</span>
                 <div className="text-xl font-black text-slate-800 mt-1">{sealedCount} / {activeBatch.items.filter(i => i.qcStatus === 'QC Passed' || i.itemStatus === 'Sealed').length} Units Secured</div>
              </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-800 text-white text-[10px] font-black uppercase tracking-[2px]">
                    ...
                    <th className="p-5 border-r border-slate-700/50">Classification</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {itemsToSeal.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="p-20 text-center text-slate-400 font-bold italic uppercase tracking-widest">No QC-Passed Items Available for Sealing</td>
                    </tr>
    ...
    <div className="flex flex-col h-full bg-slate-50 w-full animate-in fade-in duration-300 font-sans">
      <div className="p-6 pb-6 flex justify-between items-center bg-white border-b border-slate-200">
        <div className="flex items-center gap-4">
           ...
           <div>
              <h2 className="text-xl font-black text-slate-800 tracking-tight uppercase leading-none">Security Sealing Registry</h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[2px] mt-1.5">Tamper-Evident Logs</p>
           </div>
        </div>
        <div className="flex items-center gap-4">
           ...
           <table className="w-full text-left border-collapse table-fixed">
              <thead>
                <tr className="neat-table-header">
                  ...
                  <th className="p-4 w-[35%]">QC Passed Units</th>
...
function SealingRow({ item, batchNum, isBatchPosted, updateFn }) {
  ...
  return (
    <tr className={`hover:bg-slate-50 transition-colors ${!isEditing ? 'bg-sky-50/5' : ''}`}>
      ...
      <td className="p-5">
        <select value={sealType} onChange={e => setSealType(e.target.value)} disabled={!isEditing || isLocked} className={`w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-sky-500 ${(!isEditing || isLocked) ? 'bg-slate-50 text-slate-400' : ''}`}>
